...
export default function BusinessInsights() {
  ...
  const loadMetrics = async () => {
    try {
      setLoading(true)
      const response = await apiFetch(`/api/analytics/business?range=${dateRange}`)
  ...
  const exportReport = async (format: 'csv' | 'pdf' | 'excel') => {
    try {
      const response = await apiFetch(`/api/analytics/export?format=${format}&range=${dateRange}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Business Insights</h2>
          <p className="text-gray-600 dark:text-gray-400">Comprehensive analytics and data</p>
        </div>
