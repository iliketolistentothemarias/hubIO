...
export default function ChatWindow({
  ...
  useEffect(() => {
    ...
    // Single realtime path (subscribeToMessages uses postgres_changes + loads full row with sender)
    const unsubscribeMessages = messagingService.subscribeToMessages(
      conversation.id,
      (message) => {
        setMessages(prev => {
          if (prev.some(m => m.id === message.id)) return prev
          // Remove optimistic temp row when the real INSERT arrives (avoids duplicate bubbles)
          ...
          return [...withoutMatchingTemp, message]
        })
  ...
  // Fallback polling: catches messages even when Realtime/broadcast is unavailable.
  // Also polls immediately when the tab regains focus (user returns after being away).
  useEffect(() => {
    let cancelled = false
    ...
    const poll = async () => {
      ...
      if (cancelled) return
      try {
        const latest = await messagingService.getMessages(conversation.id, { limit: 80 })
        if (cancelled) return
        setMessages((prev) => {
          ...
          // Auto-scroll if a new message arrived and user is near bottom
          if (shouldAutoScroll.current) {
            setTimeout(() => scrollToBottom(true), 50)
          }
          return sorted
        ...
        /* ignore — Realtime subscription is primary; polling is just a safety net */
      }
    }
    ...
    // Poll immediately when the tab becomes visible again (user switches back to tab)
    const handleVisibility = () => {
      if (!document.hidden) poll()
    }
    // Poll immediately when the window gains focus
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // 2. Send via API so server can Realtime-broadcast to every participant (RLS does not filter broadcast)
      const res = await apiFetch(`/api/messages/${conversation.id}`, {
        method: 'POST',
