...
export default function ConversationList({
  ...
  // Live updates: postgres_changes (RLS-filtered) + server broadcast on inbox-live (all participants)
  const mergeIncomingMessage = useCallback(
    async (messageId: string) => {
      ...
      const incoming = full as Message
      ...
      setConversations((prev) => {
        ...
        const conv = { ...prev[idx] }
        if (conv.last_message?.id === incoming.id) {
          return prev
        }
        conv.last_message = incoming
        conv.updated_at = incoming.created_at

        const fromOther = incoming.sender_id !== currentUserId
        const notInOpenChat = incoming.conversation_id !== selectedConversationId
        if (fromOther && notInOpenChat) {
  ...
  useEffect(() => {
    ...
    const createChannel = () => {
      ...
      const ch = supabase
        .channel(`inbox-live:${currentUserId}`)
        .on(
          'postgres_changes',
          {
            event: 'INSERT',
          ...
          async (payload) => {
            ...
            await mergeIncomingMessage(row.id)
          }
        )
        .on(
          'broadcast',
          { event: 'inbox_message' },
          async (payload) => {
            const p = payload.payload as { messageId?: string } | undefined
            const mid = p?.messageId
            if (!mid) return
            await mergeIncomingMessage(mid)
          }
        )
        .subscribe((status, err) => {
          if ((status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') && !removed) {
            // Auto-reconnect after a brief pause — loadConversations poll bridges the gap
            setTimeout(() => {
  ...
  // Clear unread badge locally when a thread is open (ChatWindow marks read in DB)
  useEffect(() => {
      ...
      prev.map((c) =>
        c.id === selectedConversationId
          ? {
              ...
              metadata: {
                unread_count: 0,
                muted: c.metadata?.muted ?? false,
                archived: c.metadata?.archived ?? false,
                pinned: c.metadata?.pinned ?? false,
                last_read_at: c.metadata?.last_read_at,
              },
            }
          : c
      )
