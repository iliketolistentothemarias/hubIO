...
export default function Messaging({ minimized = false, onMinimize }: MessagingProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/messages/${activeConversation.id}`, {
        method: 'POST',
      ...
      const data = await response.json()
      if (data.success) {
        // Realtime subscription adds the row; if it doesn't fire, append so send always shows without reload
        setMessages((prev) =>
