...
function extractBroadcastMessageId(payload: unknown): string | undefined {
  if (payload == null || typeof payload !== 'object') return undefined
  const p = payload as Record<string, unknown>
  const inner = p.payload
  if (inner && typeof inner === 'object' && inner !== null && 'id' in inner) {
    return String((inner as { id: string }).id)
  }
...
class MessagingService {
  ...
  /** Resolves when Realtime channel is SUBSCRIBED — required before broadcast.send() works reliably */
  private channelSubscribeReady: Map<string, Promise<void>> = new Map()
  ...
  async getConversations(): Promise<Conversation[]> {
    try {
      ...
      // Latest message per conversation (global sort then take first per id — avoids wrong preview)
      ...
      const lastMessageByConv = new Map<string, (typeof allRecentMessages)[0]>()
  ...
  async sendMessage(
    ...
    try {
      ...
      void this.broadcastNewMessage(conversationId, (data as Message).id)
  ...
  private channelNameForConversation(conversationId: string) {
    return `conversation:${conversationId}`
  }

  /**
   * Load a single message row with sender join (used by realtime handlers).
   */
  ...
  /**
   * Notify other clients in this conversation that a new message exists.
   * Uses the existing subscribed channel to send a broadcast (self: true means sender also receives).
   * The server-side HTTP broadcast is the primary delivery path; this is a secondary client-side path.
   */
  async broadcastNewMessage(conversationId: string, messageId: string): Promise<void> {
    const name = this.channelNameForConversation(conversationId)
    try {
      const ready = this.channelSubscribeReady.get(name)
      if (ready) {
        await Promise.race([
          ready,
          new Promise<void>((_, rej) => setTimeout(() => rej(new Error('subscribe timeout')), 5000)),
        ]).catch(() => {})
      }
      ...
      // Non-critical: server-side HTTP broadcast and polling are the primary delivery paths
    }
  }

  /**
   * Subscribe to new messages in a conversation.
   * Listens via postgres_changes (requires Realtime enabled on the messages table) AND
   * broadcast (works regardless, used by server HTTP broadcast).
   * Auto-reconnects if the channel errors.
   */
  subscribeToMessages(
    conversationId: string,
    ...
    const deliver = async (messageId: string) => {
      if (unsubscribed) return
      const data = await this.fetchMessageById(messageId)
      if (data && !unsubscribed) callback(data)
    }
    ...
    const connect = () => {
      ...
      const stale = this.channels.get(name)
      ...
      const channel = supabase
        ...
        .on('broadcast', { event: 'new_message' }, async (payload) => {
          const id = extractBroadcastMessageId(payload)
          if (id) await deliver(id)
        })
      ...
      const ready = new Promise<void>((resolve, reject) => {
        channel.subscribe((status, err) => {
          if (status === 'SUBSCRIBED') {
            ...
            // Auto-reconnect after brief delay — polling bridges the gap
            if (!unsubscribed) {
      ...
      this.channelSubscribeReady.set(name, ready)
    ...
    return () => {
      ...
      this.channelSubscribeReady.delete(name)
    }
  }
